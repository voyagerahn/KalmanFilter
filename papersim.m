...
noiseg = randn(1,iter) * gsd; % creating the 
% fluctuations for actualg 
...
%HJK added for moving average filter
...
hold on 
plot(yS,'b','Linewidth',1.5); 
plot(HeightFromKF,'r','Linewidth',1.5); 
plot(ytr, 'k','Linewidth',1.5); 
hold off; grid; 
%title('True height, KF-estimated height and rangefinder values'); 
title('True altitude, KF-estimate and sensor values'); 
ylabel('meters') 
xlabel('Kalman Filter iterations'); 
legend('z','y-13term','KF-estimate','ytrue','Location','southwest'); 
